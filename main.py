from groq_utils import generate_response
def reinforcement_learning_activity():
    print("Welcome to the Reinforcement Learning Activity!")
    prompt = input("Please enter a prompt for the AI model: ").strip()
    if not prompt:
        print("Prompt cannot be empty. Please try again.")
        return
    
    initial_response = generate_response(prompt, temperature=0.7, max_tokens=512)
